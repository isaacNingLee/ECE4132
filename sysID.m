...
for i = [1:1:length(inputData)]
    ...
    cost = @(theta) sum((output_data-lsim(tf(theta(2), [theta(1),1]), input_data, ts)).^2);
    ...
    %theta_init = [1,1];
    thetaOpt = thetaOpt + fminsearch(cost, theta_init);

end
thetaOpt = thetaOpt / length(inputData);
vehicleModel = tf(thetaOpt(2), [thetaOpt(1),1]);
