...
% save('first_order.mat','RMSE_arr')

%% Plottting
figure();
sgtitle('NRMSE Distribution for first order model');

figure();
histogram(mean(RMSE_arr, 2), 10);

title('Average NRMSE across 4 models');
xlabel('NRMSE');
ylabel('Frequency');

% Display summary statistics as text
text(0.7, 0.9, ['Min: ', num2str(min(mean(RMSE_arr, 2)))], 'Units', 'normalized');
text(0.7, 0.8, ['Max: ', num2str(max(mean(RMSE_arr, 2)))], 'Units', 'normalized');
text(0.7, 0.7, ['Mean: ', num2str(mean(mean(RMSE_arr, 2)))], 'Units', 'normalized');
text(0.7, 0.6, ['Std Dev: ', num2str(std(mean(RMSE_arr, 2)))], 'Units', 'normalized');