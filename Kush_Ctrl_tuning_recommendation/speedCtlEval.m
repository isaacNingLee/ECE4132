...
load('task3.mat')
fprintf("Speeding fine: %d",sum(output(:,3)))

disp(newline+"NMSE data")
fprintf("Maximum: %.4f",max(output(:, 2)))
fprintf(newline+"Mean: %.4f",mean(output(:, 2)))
fprintf(newline+"Std_dev: %.4f",std(output(:, 2)))

disp(newline+' ')
disp(newline+"Acc data")
fprintf("Maximum: %.4f",max(output(:, 4)))
fprintf(newline+"Mean: %.4f",mean(output(:, 4)))
fprintf(newline+"Std_dev: %.4f",std(output(:, 4)))

disp(newline+' ')
disp(newline+"Jerk data")
fprintf("Maximum: %.4f",max(output(:, 5)))
fprintf(newline+"Mean: %.4f",mean(output(:, 5)))
fprintf(newline+"Std_dev: %.4f",std(output(:, 5)))
%% Plotting
data1 = output(:, 2);
data2 = output(:, 4);
data3 = output(:, 5);
...
% between the measured and reference velocity and produces as output a 
% torque request for the vehicle model.
[LTIblock,Ki,Kt,Tmax,Tmin] = ctlDesign_ZN_AMIGO(Gvehicle,task);
